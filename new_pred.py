...
def replace_outliers(df, column_to_filter):
    
    # Ensure the column is numeric and has no NaNs
    df[column_to_filter] = pd.to_numeric(df[column_to_filter], errors='coerce').fillna(df[column_to_filter].median())
    ...
    lower = Q1 - 1.5 * IQR
    upper = Q3 + 1.5 * IQR
    med = df[column_to_filter].median()
    
    df.loc[(df[column_to_filter] < lower) | (df[column_to_filter] > upper), column_to_filter] = med
